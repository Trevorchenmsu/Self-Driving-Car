...
int main() {
	...
	h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &behavior]
				...
				if (event == "telemetry") {
					...
					for ( int i = 0; i < sensor_fusion.size(); i++ ) {
						...
						if ( d >= 0 && fabs(car_s - s) < 100 ) {
							vector<Prediction> predictions =  Prediction::predict(x, y, vx, vy, s, d,
					...
					vector<vector<double>> xy = behavior.calculate_behavior(car_x, car_y, car_s, car_d, car_speed, car_yaw,
					...
					std::cout << "car: x: "     << car_x 
						      ...
						      << " car: s: "     << car_s
							  << " car: d: "     << car_d
							  << " previous path size: " << previous_path_x.size()
							  << "\n";
