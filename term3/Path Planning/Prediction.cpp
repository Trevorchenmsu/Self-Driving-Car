...
/**
 * Naive Bayes Algorithm to determine the probability of a behavior (keep lane,change left,
 * change right) given the d coordinate. I assume the three states have equal probability.
 * If the states have different probabilities then the variables pl, pm and pr should be
 * multiplied by the corresponding probabilities of change left, keep lane and change right.
 * 
 * For the probability p(d|state) I use a Gaussian distribution with mean on the left side,
 * middle and right side of the lane, and a standard deviation of 0.5 for the sides and 0.6 for the center.
 */
std::vector<Prediction> Prediction::predict(double x, double y, double vx, double vy, double s, double d,
	...
	// the mean on the left, middle, right sides of the lane
