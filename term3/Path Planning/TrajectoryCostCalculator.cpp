...
// Slowest car ahead within 45m in the given lane. TARGET_SPEED if no cars ahead.
	...
	for (auto vehicle_predictions = predictions.begin(); vehicle_predictions < predictions.end(); ++vehicle_predictions) {
		...
		for (auto prediction = vehicle_predictions->begin(); prediction < vehicle_predictions->end(); ++prediction) {
			...
			if ( Trajectory::get_lane( vehicle_d ) == lane ) {
				...
				// same lane. s is the coordinate of ego car
				if (s < vehicle_s && vehicle_s < s + 45.0 ) { // in the dangerous area, we need to slow down

					if ( prediction->getTrajectory().getTargetSpeed() < max_speed) { // dectected car is slower than ego car
						max_speed = prediction->getTrajectory().getTargetSpeed();  // maintain the same speed as the car ahead.
					}
				}
			}
		}
	...
	for (auto vehicle_predictions = predictions.begin(); vehicle_predictions < predictions.end(); ++vehicle_predictions) {
		for (auto prediction = vehicle_predictions->begin(); prediction < vehicle_predictions->end(); ++prediction) {
			...
			for ( int i = 0; i < NO_POINTS; i++ ) {
				...
				if ( fabs( prediction->getTrajectory().getD()[i] -  ego_trajectory.getD()[i]) < 2) {
...
double road_limits(Trajectory ego_trajectory, vector<vector<Prediction>> predictions) {
	assert( ego_trajectory.getD().size() == NO_POINTS );
	for ( int i = 0; i < NO_POINTS; i++ ) {
		if ( ego_trajectory.getD()[i] <= 1.0 ||
				ego_trajectory.getD()[i] >= LANE_WIDTH * (MAX_LANE + 1) - 1.0 ) {
			return 1.0;
		}
	}
	return 0.0;
...
double inefficiency_cost(Trajectory ego_trajectory, vector<vector<Prediction>> predictions) {
	/*
	...
	*/
	...
	cout << "inefficiency_cost target_lane_speed=" << target_lane_speed << "\n";
	cout << "inefficiency_cost final_lane_speed=" << final_lane_speed << "\n";
	cout << "inefficiency_cost TARGET_SPEED=" << TARGET_SPEED << "\n";
