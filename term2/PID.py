...
def run(param1, param2, param3):
    ...
    # 10 degree systematic bias, this will be added in by the move function, you do not need to add it below!
...
def twiddle(tol=0.2):
    ...
    dp = [1, 1, 1]
    myrobot = robot()
    x_trajectory, y_trajectory, best_err = run(myrobot, p)
    ...
    while sum(dp) > tol:
        ...
        for i in range(len(p)):
            p[i] += dp[i]
            myrobot = robot()
            x_trajectory, y_trajectory, err = run(myrobot, p)
